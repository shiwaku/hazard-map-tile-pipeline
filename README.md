...
3 つとも公開データで、`auto` 任せ（`TILE_TYPE` を除く）で通ることを確認している。

| | `ksj-flood-ujigawa` | `fukushima-depth-numeric` | `fukushima-depth-palette` |
|---|---|---|---|
| 元データ | 国土数値情報 A31a 2025年度 | 福島県 津波浸水想定 | 同左 |
| 対象 | 高知県 宇治川（想定最大規模） | 地域海岸9 最大浸水深 | 同左 |
| 値属性 | `A31a_205`（Integer） | `z`（Real） | `z`（Real） |
| 値の種別 | rank（1〜4 の4種類） | depth（0.01〜14.26 m） | depth |
| タイル種別 | palette（**強制**） | numeric | palette（明示指定） |
| CRS | EPSG:6668（地理座標系） | EPSG:2451（平面直角IX系） | 同左 |
| メッシュ | 5m（経度0.225″×緯度0.15″） | 10m（X10m×Y10m） | 同左 |
| 最大ZL | 18（ネイティブ15＋3） | 17（ネイティブ14＋3） | 17 |
| フィーチャ数 | 1,547 | 9,274 | 9,274 |
| 元データ | 1.5 MB | 1.5 MB | 1.5 MB |
| 生成タイル | 328 枚 / 1.1 MB | 117 枚 / 412 KB | 117 枚 / 364 KB |
| 所要時間 | 7 秒 | 4 秒 | 4 秒 |
| 検証 | PASS（凡例の6色中4色が出現） | PASS（復号値域 0.01〜14.26） | PASS（凡例の8色中7色が出現） |

`fukushima-depth-*` の 2 つは**同じ入力から別形式を出している**。実数値なら
どちらも選べることの実例で、設定の差は `TILE_TYPE` と `RANK_DEF` の 2 行だけ。
