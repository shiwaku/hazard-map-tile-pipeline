...
> **公開データの実情**: 洪水浸水想定区域の公開データは、国土数値情報も多くの
> 都道府県公開分も浸水深をランクコードで持っている。実数値の原典は浸水想定区域図データ
> 電子化ガイドラインの `MAXALL`（最大包絡）で、**都道府県が自らの管理河川について
> これをそのまま公開している例がある**（同梱サンプルの石川県はこれ）。
> 一方、利根川・荒川のような**国管理の大河川の本川は実数値が公開されていない**ため、
> 「実数値」と「大河川」は公開データでは両立しない。
> パイプラインは災害種別を区別しない（入力の性質だけで分岐する）。
...
CI は国土数値情報（`nlftp.mlit.go.jp`）と石川県サイトから ZIP を取りに行く。
...
| | `ksj-flood-ujigawa` | `ksj-duration-ujigawa` | `ishikawa-saigawa-numeric` | `ishikawa-saigawa-palette` |
|---|---|---|---|---|
| 元データ | 国土数値情報 A31a 2025年度 | 同左（**同じZIP**） | 石川県 洪水浸水想定区域図オープンデータ | 同左（**同じ入力ファイル**） |
| 対象 | 高知県 宇治川 浸水深（想定最大規模） | 高知県 宇治川 浸水継続時間 | 石川県 犀川（想定最大規模） | 同左 |
| 値属性 | `A31a_205`（Integer） | `A31a_305`（Integer） | `浸水深`（Real） | `浸水深`（Real） |
| 値の種別 | rank（1〜4 の4種類） | rank（1〜3 の3種類） | depth（0.01〜6.41 m） | depth |
| タイル種別 | palette（**強制**） | palette（**強制**） | numeric | palette（明示指定） |
| ランク定義 | `water-depth-6rank` | `flood-duration-7rank` | —（凡例を持たない） | `water-depth-8rank` |
| CRS | EPSG:6668（地理座標系） | 同左 | EPSG:4612（JGD2000） | 同左 |
| メッシュ | 5m（経度0.225″×緯度0.15″） | 10m（経度0.45″×緯度0.3″） | 5m（経度0.2232″×緯度0.1512″） | 同左 |
| 最大ZL | 18（ネイティブ15＋3） | 17（ネイティブ14＋3） | 18（ネイティブ15＋3） | 18 |
| フィーチャ数 | 1,547 | 1,125 | 1,397,147 | 1,397,147 |
| 元データ容量 | 1.5 MB | 同左 | 238 MB（ZIP 37 MB） | 同左 |
| 生成タイル | 328 枚 / 1.1 MB | 112 枚 / 368 KB | 4,001 枚 / 23 MB | 4,001 枚 / 16 MB |
| 所要時間 | 7 秒 | 3 秒 | 115 秒 | 114 秒 |
| 検証 | PASS（凡例の6色中4色が出現） | PASS（7色中3色が出現） | PASS（復号値域 0.01〜6.41） | PASS（8色中6色が出現） |

### `ishikawa-saigawa-*` — 同じ入力から別形式を出す

設定の差は `TILE_TYPE` と `RANK_DEF` の 2 行だけ。**実数値の入力なら数値PNGも
パレットPNGも選べる**ことの実例になっている。同じ画素をクリックすると、
numeric は `2.34 m` のような実数を、palette は `1.0〜3.0m未満` を返す。

パレットPNGにすると**実数は復元できなくなる**（2.34 という値は色に潰れる）。
値そのものを配りたいなら numeric、地図として読ませたいなら palette を選ぶ。
なお numeric は色に意味が無いので、そのまま重ねても地図としては読めない
（画面上は青一色に見える）。

洪水で実数値の浸水深が手に入る公開データは限られる。国土数値情報も多くの
都道府県公開分もランクコードしか持たず、実数値の原典である `MAXALL`（最大包絡）は
都道府県が自らの管理河川について公開している例があるだけで、利根川・荒川のような
**国管理の大河川の本川は公開されていない**。石川県は 27 河川ぶんの `MAXALL` を
シェープファイルのまま出しているので、そのまま通せる。
...
│   ├── ishikawa-saigawa-numeric.conf.example… 実数値 → 数値PNG
│   └── ishikawa-saigawa-palette.conf.example… 同じ入力を → パレットPNG
...
| [洪水浸水想定区域図オープンデータ](https://www.pref.ishikawa.lg.jp/kasen/sinsui-m/opendata.html)（犀川・想定最大規模） | 石川県 | 「オープンデータ」として公開。CC 等の明示は無い（下記） |

石川県のデータは、県が「オープンデータ」として公開しているもので、県のオープンデータの
定義も「営利目的、非営利目的を問わず二次利用可能なルールが適用されたもの」としているが、
配布ページにも県のサイトポリシーにも CC ライセンスの明示が無い。出典を明記して利用している。
明示ライセンスが要る用途では、[東京都の浸水予想区域図（浸水深・地盤高データ、CC BY）](https://catalog.data.metro.tokyo.lg.jp/dataset/t000014d0000000029)
や福島県の津波浸水想定（CC BY 2.1 JP）が同じ MAXALL 系のデータとして使える。
